...
class _FreeItemsPageState extends State<FreeItemsPage> {
  ...
  void _onAddPressed() {
    final resp = packagesController.freeAddOnResponse.value;
    if (_selectedIndex != -1 && resp != null && _selectedIndex < resp.addons.length) {
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Container(
          ...
          child: Obx(() {
            final loading = packagesController.isFetchingFreeAddOns.value;
            final resp = packagesController.freeAddOnResponse.value;
            final addons = resp?.addons ?? [];
            final mainTitle = (resp?.mainTitles.isNotEmpty == true) ? (resp?.mainTitles.first ?? "") : "Free Add-Ons";
                            ...
                            ClipRRect(
                              ...
                              child:
                                  item.coverImage != null && item.coverImage!.isNotEmpty
                                      ...
                                      : Container(height: 50, width: 50, color: Colors.grey[200], child: const Icon(Icons.image)),
                            ),
