...
class _CompanyListScreenState extends State<CompanyListScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (companyController.selectedCategoryId != widget.categoryId || companyController.companies.isEmpty) {
            ...
            return GridView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return GestureDetector(
                  onTap: () {
                    log('Navigating to VendorDescription for company: ${company.name}');
                    final mainState = context.findAncestorStateOfType<NavigationFlowState>();
                    // Get.to(() => CompanyLocatorMapScreen(categoryId: cat.id));
                    mainState?.pushToCurrentTab(VendorDescription(company: company, companyId: company.companyId), hideBottomBar: true);
                  },
                  ...
                  child: VendorRectCard(
                    ...
                    rating: company.company?.rating?.toStringAsFixed(2) ?? company.rating?.toStringAsFixed(2) ?? '3.9',
                    estimatedTime: "${company.estimatedTime} · ",
                    distanceKm:
                        company.distanceKm != null
                            ? (company.distanceKm! < 1 ? (company.distanceKm! * 1000).toStringAsFixed(0) : "${company.distanceKm!.toStringAsFixed(1)} km")
                            : null,
                  ),
                );
              },
            ...
            child: Stack(
              children: [
                ClipRRect(
                  ...
                  child: CustomImage(path: logoImage.isNotEmpty ? logoImage : 'assets/images/placeholder.jpg', height: itemWidth * 1.5, width: double.infinity, fit: BoxFit.cover),
                ),
                Positioned(
                  top: 8,
                  right: 8,
                  child: GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: GetBuilder<WishlistController>(
                        builder: (controller) {
                          return Icon(Icons.favorite, size: 25, color: controller.bookmarks.any((element) => element.id == vendorId) ? Colors.red : Colors.grey);
                        },
                      ),
                    ),
                  ),
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                      ...
                      Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            Icon(Icons.star, size: itemWidth * 0.075, color: const Color(0xffF8DE1E)),
                          ],
                        ),
                      ),
                  ],
                ...
                Row(
                  children: [
                    Icon(Icons.location_on, size: itemWidth * 0.085, color: const Color(0xffB8B7C8)),
                    SizedBox(width: itemWidth * 0.025),
                    Expanded(
                      child: Row(
                        children: [
                          Text(estimatedTime ?? 'N/A', style: TextStyle(fontSize: itemWidth * 0.065, color: const Color(0xffB8B7C8))),
                          Text(distanceKm ?? 'N/A', style: TextStyle(fontSize: itemWidth * 0.065, color: const Color(0xffB8B7C8)), overflow: TextOverflow.visible),
                        ],
                      ),
                    ),
                  ],
