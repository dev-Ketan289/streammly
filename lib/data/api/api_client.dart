...
class ApiClient extends GetConnect implements GetxService {
  ...
  ApiClient({required this.appBaseUrl, required this.sharedPreferences}) {
    try {
      ...
      _mainHeaders = {'Accept': 'application/json', 'Authorization': 'Bearer $token'};
    } catch (e) {
  ...
  Future<Response> getData(String uri, {Map<String, dynamic>? query, String? contentType, Map<String, String>? headers, Function(dynamic)? decoder}) async {
    try {
      ...
      Response response = await get(uri, contentType: contentType, query: query, headers: headers ?? _mainHeaders, decoder: decoder);
  ...
  Future<Response> postData(
    ...
    try {
      ...
      Response response = await post(uri, body, query: query, contentType: contentType, headers: headers ?? _mainHeaders, decoder: decoder, uploadProgress: uploadProgress);
      JsonEncoder jsonEncoder = const JsonEncoder();
      // ApiChecker.checkApi(response);
      log('====> GetX Response: [${response.statusCode}] $uri\n${jsonEncoder.convert(response.body)}');
      response = handleResponse(response);
  ...
  Future<Response> putData(
    ...
    try {
      ...
      Response response = await put(uri, body, query: query, contentType: contentType, headers: headers ?? _mainHeaders, decoder: decoder, uploadProgress: uploadProgress);
      response = handleResponse(response);
      log('====> GetX Response: [${response.statusCode}] $uri\n${response.body}');
      if (kDebugMode) {
  ...
  Future<Response> patchData(
    ...
    try {
      ...
      Response response = await patch(uri, body, query: query, contentType: contentType, headers: headers ?? _mainHeaders, decoder: decoder, uploadProgress: uploadProgress);
      response = handleResponse(response);
      log('====> GetX Response: [${response.statusCode}] $uri\n${response.body}');
      if (kDebugMode) {
        // print('====> GetX Response: [${response.statusCode}] $uri\n${response.body}');
  ...
  Future<Response> deleteData(String uri, {Map<String, dynamic>? query, String? contentType, Map<String, String>? headers, Function(dynamic)? decoder}) async {
    try {
      ...
      Response response = await delete(uri, headers: headers ?? _mainHeaders, contentType: contentType, query: query, decoder: decoder);
      response = handleResponse(response);
      log('====> GetX Response: [${response.statusCode}] $uri\n${response.body}');
      if (kDebugMode) {
        // print('====> GetX Response: [${response.statusCode}] $uri\n${response.body}');
  ...
  Future<dynamic> commonApiCall(String urlR, Map<String, dynamic> body) async {
    ...
    try {
      ...
      log(
        ...
        name: "API ERROR CAC(/${"${postUri.path.split('/').last} ============ " /*+ getResponseStatus(response)*/})",
        level: 1,
      );
      return null;
    }
